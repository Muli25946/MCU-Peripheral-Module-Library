...
#define W25QXX_BLOCK_SIZE         65536                                     /**< 每块字节数 */
#define W25QXX_PAGES_PER_SECTOR   (W25QXX_SECTOR_SIZE / W25QXX_PAGE_SIZE)  /**< 每扇区页数：16 */
#define W25QXX_SECTORS_PER_BLOCK  (W25QXX_BLOCK_SIZE / W25QXX_SECTOR_SIZE) /**< 每块扇区数：16 */

/**< 由块-扇区-页-页内偏移计算字节地址 */
#define W25QXX_ADDR(block, sector, page, offset) \
  ((uint32_t)(block)  * W25QXX_BLOCK_SIZE  + \
   (uint32_t)(sector) * W25QXX_SECTOR_SIZE + \
   (uint32_t)(page)   * W25QXX_PAGE_SIZE   + \
   (uint32_t)(offset))
